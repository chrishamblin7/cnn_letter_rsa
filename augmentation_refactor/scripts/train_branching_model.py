...
output_name = 'branch4_letter_mixedaug'   # base name for various outputs, like the print log and the saved model
device = 'cuda:2'   #use GPU acceleration
branch_point = '9'      #the name of the module your branching net is going to branch from (take the outputs from). 
...
acc_thresh = .9     #how good does the model need to be to save?
...
def test(model, device, test_loader, criterion, epoch, max_acc):
	...
	if ((epoch-1)%save_model_interval == 0 or total_acc > acc_thresh) and save_model == True:       #condition to save model
		if not save_only_best:
			print('SAVING MODEL')
			torch.save(model,'../models/branchingnet/%s_acc%s.pt'%(output_name,str(round(total_acc,3))))
		elif total_acc > max_acc:
			#if os.path.exists('../models/branchingnet/%s_acc%s.pt'%(output_name,str(round(max_acc,3)))):
			#	call('rm ../models/branchingnet/%s_acc%s.pt'%(output_name,str(round(max_acc,3))),shell=True)
			print('SAVING MODEL')
			torch.save(model,'../models/branchingnet/%s_acc%s.pt'%(output_name,str(round(total_acc,3))))			
			max_acc = total_acc
...
if __name__ == '__main__':
	...
	test_loader = torch.utils.data.DataLoader(
		data_classes.augmentation_letters_data(train=False,use_augment=False),
		batch_size=batch_size, shuffle=True, **kwargs)
	...
	for epoch in range(1, epochs + 1):
		model,optimizer = train(model,device, train_loader, optimizer,scheduler, criterion, epoch)
