...
subfolder = 'alexnet'        #(branchingnet,alexnet,smallnet)

output_names = ['alexnet_mixed_halfaug_ep21_imnet0.555_letter0.939']
...
#data loader
labels = data_classes.letter_labels

mixedletter_transform = transforms.Compose([
				transforms.Resize((224,224)),
				transforms.ToTensor()
				])

def image_loader(image_folder, transform=mixedletter_transform):
...
for output_name in output_names:
	...
	for label in labels:
		print(i)
		i+=1
		data = image_loader(os.path.join('../../data/mixed_imnet_letter/test/',label))
	...
	if output_type == 'mat':
		scipy.io.savemat('../activations/%s/%s_lettertest_activations.mat'%(subfolder,output_name), activations)
		#scipy.io.savemat('../activations/%s/%s_output.mat'%(subfolder,output_name), dict(output=output.detach().numpy()))
	else:
		pickle.dump(activations, open('../activations/%s/%s_lettertest_activations.pkl'%(subfolder,output_name),'wb'))
